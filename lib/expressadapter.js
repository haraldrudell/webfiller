...
function addRoutes(defaults, app, callback) {
	...
	var viewFolder = app.settings && app.settings.views
	if (!viewFolder) throw Error('Can not find view folder')
	var defExt = app.settings && app.settings['view engine'] || 'html'
	var webFillerFolder = path.join(viewFolder, '../public/webfiller')
	webfillerinit.initWebFiller(defaults, viewFolder, defExt, webFillerFolder, function (err, routes) {
		if (!err) {
