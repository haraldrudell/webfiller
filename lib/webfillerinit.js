...
function initWebFiller(defaults, opts, callback) {
	...
	var cacheFlag = false // TODO what to do here?
	viewloader.setParameters(opts.viewFolder, opts.defExt, false)
	var views = topLevelViews(opts.viewFolder, opts.defExt)
	...
	var fds = {
		css: {
			wrote: false,
			fd: fs.openSync(path.join(opts.webFillerFolder, 'index.css'), 'w')
		}, js: {
			wrote: false,
			fd: fs.openSync(path.join(opts.webFillerFolder, 'index.js'), 'w')
		}
	}
	...
	function writeFragment(fragment, fragmentName) {
		if (!WF.fragments[fragmentName]) {
			var absolute = path.join(opts.viewFolder, fragment.view + '.' + opts.defExt)
