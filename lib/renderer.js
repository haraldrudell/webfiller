	...
	*/
	function render(record, viewExecutable, mainView) {
		...
		if (typeof viewExecutable == 'string') {
			var ve = fragResolver(viewExecutable, mainView)
			if (!ve) return 'Unknown fragment:' + viewExecutable
			viewExecutable = ve
		}
				...
				for (var funcName in dataSource) {
					...
					if (funcName == 'fragment') {
						...
						if (typeof params == 'string') {
							var renderFunction = fragResolver(params, this.domain)
							if (!renderFunction && isNode) {
								renderFunction = require('./compiler').loadFragment(params, this.domain)
							}
							if (renderFunction instanceof Function) {
								...
								this.printRaw(data)
							} else this.error('Unknown view:' + params)
	...
	function fragResolver(fragmentName, domain) {
		var result

		fragmentName = String(fragmentName)
		if (~fragmentName.indexOf('.')) {
			// specific domain, get it directly
			result = WF.fragments[fragmentName]
		} else {
			// try domain first, then any
			if (typeof domain == 'string') result = WF.fragments[fragmentName + '.' + domain]
			if (!result) { // now scan...
				var target = fragmentName + '.'
				var len = target.length
				for (var frag in WF.fragments) {
					if (frag.substring(0, len) == target) {
						result = WF.fragments[frag]
						break
					}
				}
			}
